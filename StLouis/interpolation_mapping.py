...
boxes_data['centroid'] = boxes_data.geometry.centroid
...
for year in years:
	...
	if year != years[0]:
		...
		boxes_data['color'] = boxes_data['color2'].values
		...
		for bind in boxes_data.index:
			...
			bc = boxes_data.loc[bind].centroid

			dist_counter = 0
			lowest_distances = []
			color_values = []
			for tind in data.index:

				tc = data.loc[tind].centroid

				dist = haversine((bc.x,bc.y),(tc.x,tc.y))

				if dist_counter<4:
					lowest_distances.append(dist)
					color_values.append(data.loc[tind][dem_color])
					...
					if dist < min(lowest_distances):
						min_loc = np.argmin(lowest_distances)
						lowest_distances[min_loc] = dist
						color_values[min_loc] = data.loc[tind][dem_color]

				dist_counter = dist_counter + 1

			weights = 1.0 / lowest_distances
			color_val = np.sum(color_values*weights) / np.sum(weights)
			boxes_data['color'].loc[bind] = color_val
	...
	boxes_data.plot(column='color',cmap='gray', ax=ax1[0], legend=False, edgecolor='face', linewidth=0.25)
	...
	if year!=years[-1] and interpolate==True:
		...
		data2['centroid'] = data2.geometry.centroid
		...
		for bind in boxes_data.index:
			...
			lowest_distances = []
			color_values = []
			for tind in data2.index:

				tc = data2.loc[tind].centroid

				dist = haversine((bc.x,bc.y),(tc.x,tc.y))

				if dist_counter<4:
					lowest_distances.append(dist)
					color_values.append(data2.loc[tind][dem_color])
					...
					if dist < min(lowest_distances):
						min_loc = np.argmin(lowest_distances)
						lowest_distances[min_loc] = dist
						color_values[min_loc] = data2.loc[tind][dem_color]

				dist_counter = dist_counter + 1

			weights = 1.0 / lowest_distances
			color_val = np.sum(color_values*weights) / np.sum(weights)
			boxes_data['color2'].loc[bind] = color_val
		...
		for s in range(num_steps):
			...
			## plot demographic data
			boxes_data.plot(column='color3',cmap='gray', ax=ax1[0], legend=False, edgecolor='face', linewidth=0.25)
